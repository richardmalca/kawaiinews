...
- **Caché** — escribe y lee una clave real en el driver de caché configurado (`CACHE_STORE`); `critical` si falla. Existe por el incidente real de ese mismo día: `CACHE_STORE=redis` sin el cliente Redis instalado tumbaba toda la app con `Class Redis not found`, sin ninguna señal previa — ahora se ve acá antes de que rompa otra página.

### Cachear el panel sin poder romperlo si el caché falla

`summary()`, `growth()`, `timeline()`, `topArticles()` y `categoryBreakdown()` cachean su resultado 2 minutos (`rememberSafely()`) — el panel se puede refrescar seguido y estas consultas no necesitan ser exactas al segundo. `healthChecks()` queda **afuera** a propósito: tiene que reflejar el estado real en el momento.

`rememberSafely()` envuelve `Cache::remember()` en un `try/catch`: si el driver de caché configurado falla (el mismo caso del chequeo de arriba), calcula el valor sin caché en vez de tirar un 500 — si no, agregar caché para "optimizar" el panel lo dejaría más frágil justo cuando más se necesita (durante un incidente real de infraestructura).
