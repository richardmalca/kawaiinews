...
## Separación de la configuración de cuenta: `/settings` (staff) vs `/perfil/{username}/ajustes` (lectores)
...
- **`/perfil/{username}/ajustes`** — cuenta del lector público, layout público (sin sidebar admin), URL en español como el resto del sitio público. Nuevo `App\Http\Controllers\Public\ProfileSettingsController` con `edit`/`update`/`destroy`, protegido por `auth` + verificación de dueño (`$request->user()->username === $username`, si no `403`). Usa `App\Http\Requests\Public\ProfileSettingsUpdateRequest` (mismo trait `ProfileValidationRules`, pero acá `username` es obligatorio) y `ProfileSettingsDeleteRequest`.
  - `GET perfil/mi-cuenta/ajustes` (`ProfileSettingsController::redirectToSelf`) es el alias estable para quien todavía no tiene `@usuario`: si ya eligió uno, redirige a `perfil/{username}/ajustes`; si no, debería mostrar una pantalla para elegirlo (`Inertia::render('public/profile/settings/choose-username')` — página aún no creada, es tarea de la sesión de frontend público).

Rutas nuevas en `routes/web.php`, dentro del grupo `auth`:

```
GET    perfil/mi-cuenta/ajustes         public.profile.settings.self
GET    perfil/{username}/ajustes        public.profile.settings.edit
PATCH  perfil/{username}/ajustes        public.profile.settings.update
...
- `tests/Feature/Public/ProfileSettingsTest.php` — lector sin rol no puede entrar a `/settings/profile` (403), un editor sí puede, redirección de `mi-cuenta/ajustes` según tenga o no `@usuario`, un usuario puede editar solo su propia configuración pública (403 sobre la de otro), y el username no puede colisionar con una palabra reservada.
