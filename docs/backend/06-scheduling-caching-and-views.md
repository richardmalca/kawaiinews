...
```php
Schedule::command('news:scrape')->everyThreeHours()->withoutOverlapping()->onOneServer();
Schedule::command('news:auto-review')->everyThreeHours(10)->withoutOverlapping()->onOneServer();
Schedule::command('views:flush')->everyMinute()->withoutOverlapping()->onOneServer();
```

- `news:scrape` (`ScrapeNewsCommand`) — corre `NewsScraperService::run()` directo (sin pasar por cola: un comando de consola no tiene el límite de tiempo de un request HTTP, así que no hace falta). Antes el scraping era 100% manual vía el botón "Buscar noticias ahora"; ahora corre solo cada 3 horas (8 veces al día) además de poder dispararse a mano. Se bajó de cada 30 minutos a cada 3 horas porque en 30 minutos casi no se acumulan noticias nuevas en las fuentes RSS — no vale la pena correr el scraper (ni el análisis con IA que sigue) tan seguido.
- `news:auto-review` (`AutoReviewNewsCommand`) — analiza con IA (`NewsClusterService::analyzeWithAi()`) todo cluster `pending` sin veredicto todavía, y **rechaza automáticamente** (`status = rejected`) los que la IA marcó `discard` (`NewsClusterService::autoRejectDiscarded()`). Los marcados `publish` **no** se tocan solos — siguen esperando que alguien los acepte a mano desde `/admin/news-review` (o el botón "Aceptar todo lo marcado Publicar"), a propósito: la IA filtra el ruido, pero la decisión de qué se convierte en noticia real la sigue tomando una persona. Corre 10 minutos después de cada scrape (`everyThreeHours(10)`), para darle tiempo a terminar de agrupar los clusters nuevos antes de analizarlos. Si no hay proveedor de IA activo con API key, no rompe nada — `analyzeWithAi()` ya devolvía ese caso como `analyzed: 0` con un mensaje de error, y el comando lo loguea como warning y sigue.
  - **Corre cada 3 horas indefinidamente y le pega a un proveedor de IA real** — implica un costo recurrente, no es gratis dejarlo prendido. No tiene el rate limit `ai-costly` (ver [04-admin-news-sources.md](04-admin-news-sources.md)) porque ese limitador es por usuario HTTP y esto corre por cron, sin sesión.
...
**Segundo bug real, esta vez de concurrencia:** `markPending()` (agregar un `article_id` a la lista `article-views:pending-ids`) y la lectura+borrado de esa lista/contadores en `flushPending()` eran lee-modifica-escribe sin ningún lock. Con tráfico real (dos requests para artículos *distintos* llegando casi al mismo tiempo, algo normal con PHP-FPM sirviendo en paralelo), dos llamadas podían leer la misma lista vieja y pisarse el `Cache::put()` una a la otra — el `article_id` que perdía la carrera nunca entraba a la lista, así que `flushPending()` nunca lo procesaba: esa vista quedaba huérfana en `article-views:pending:{id}` y expiraba sola a las 2hs sin volcarse **nunca** a la base, en silencio. Mismo problema en `flushPending()` al leer+borrar la lista y cada contador individual. Se envuelven las tres operaciones lee-modifica-escribe en `Cache::lock(...)->block(5, ...)` — el driver `database` implementa `LockProvider` (tabla `cache_locks`, ya en la migración base de Laravel), así que el lock es real, no cosmético.
...
- `tests/Feature/Public/ArticleViewServiceTest.php` — dedupe por visitante, que el flush no toque `updated_at`, visitantes distintos cuentan por separado, y que vistas de muchos artículos distintos lleguen todas al mismo flush sin perderse ninguna.
