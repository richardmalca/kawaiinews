...
Disparo: botón manual "Buscar noticias ahora" en `/admin/news-review`, **y** un cron cada 3 horas (`news:scrape`, ver [06-scheduling-caching-and-views.md](06-scheduling-caching-and-views.md)). El botón valida que haya al menos una fuente activa con `rss_url` antes de habilitarse — si no hay ninguna, la página muestra una alerta (`Alert` destructiva) con link directo a Fuentes de noticias y el botón queda deshabilitado; el endpoint `POST news-review/scrape` también valida esto server-side (devuelve `sources_scraped: 0` y un mensaje en `errors`, sin correr el scraper).

## Etapa B — Bandeja de revisión → CMS

### `app/Services/NewsClusterService.php`

- `reviewQueue(string $sort = 'relevance', ?string $category = null, int $perPage = 20, int $page = 1)` — clusters con `status` en `['pending', 'accepted']` (los `rejected` no se muestran nunca), opcionalmente filtrados por `category`, con `scrapedItems.newsSource` y `article` cargados. `$sort` acepta `relevance` (default, por `relevance_score`), `newest` u `oldest` — estos dos últimos ordenan por la **fecha real de la noticia** (la fecha más antigua entre los `scraped_items` del cluster, no cuándo se scrapeó), para que el orden coincida con lo que se ve en la columna "Fecha" de la tabla.
- **Paginación en SQL, no en memoria.** Versión anterior: traía TODOS los clusters `pending`/`accepted` con `->get()` y paginaba con `Collection::forPage()` en PHP, porque el orden por fecha dependía de `NewsCluster::earliestPublishedAt()` (un valor calculado desde la relación `scrapedItems`, no una columna) — no se podía usar `paginate()` de Eloquent directo. Eso traía toda la cola a memoria en cada carga de la página, sin límite, y crecía con cada scraping sin que nadie la purgara. Se resolvió con una **subquery correlacionada** (`(select min(published_at) from scraped_items where scraped_items.news_cluster_id = news_clusters.id)`, constante `EARLIEST_PUBLISHED_AT_SQL`) agregada al `select` y usada en `orderByRaw()` — funciona igual en SQLite/MySQL/Postgres sin depender de las reglas de `GROUP BY` de cada motor (a diferencia de un `JOIN` + `GROUP BY`). Con eso sí se puede usar `->paginate($perPage, page: $page)` de Eloquent normal, trayendo de la base solo los 20 registros de la página actual. El selector de orden vive en `NewsReviewSortSelect` y el de categoría en `NewsReviewCategorySelect`, ambos cambian la URL vía `router.get(..., {sort, category})` con `preserveState`; los botones Anterior/Siguiente navegan con `router.get(..., {page})` usando el `meta` (`current_page`, `last_page`, `total`) que ahora vienen directo del `LengthAwarePaginator`.
